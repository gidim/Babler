 ...
 */
public class TwitterCodeSwitchSJobManager {

    /* An api consumer for each language */
    TwitterCodeSwitchScraper [] scrapers = new TwitterCodeSwitchScraper[LanguageDataManager.getLanguages().length]; //languages
    ...
    public void start() throws InterruptedException {
        ...
        while(true) {
            for (int i = 0 ; i < langs.length ; i++) {
                ...
                TwitterCodeSwitchScraper ts = scrapers[i];
                ts.setKey(key);
                try {
